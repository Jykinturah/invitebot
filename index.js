...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    // Second Opinion
    if (reaction.emoji.name === '❓') {
        if (reaction.message.embeds[0].fields[5].name === 'Second Opinion By') {
            return;
        }
        const inviteEdit = new EmbedBuilder(reaction.message.embeds[0]).spliceFields(5, 1)
            .addFields({ name: 'Second Opinion By', value: user.tag, inline: true }, { name: 'Responses', value: '✅ Accept | 👨 Man | ℹ Request Info | 🔄 Resend Invite \n 🔥 Archive' })
...
client.on('messageCreate', async (message) => {
    ...
    if (Array.isArray(config.modID)) {
        for (let i = 0; i < config.modID.length; i++) {
            if (message.member.roles.cache.has(config.modID[i])) {
                isModId = true;
            }
        }
    } else if (message.member.roles.cache.has(config.modID)) {
        isModId = true;
    }
    ...
    if (command === 'invite') {
        if (!args.length) {
            ...
            r.getNewModmailConversation(args[1])
                .reply(args.slice(2).join(' '))
                .then(() => {
                    r.getNewModmailConversation(args[1]).archive()
                });
            message.react('🏐')
        };
    };
